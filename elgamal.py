...
def create_EG_private_key():
    # Creates an ElGamal private key
    private_key = randint(1,p-1)
    return private_key

def EG_public_key(private_key):
    # Returns the public key corresponding to an integer private key
    b = private_key
...
def EG_encrypt(plaintext, public_key):
    # ElGamal encryption of an integer plaintext with an integer public key
...
def EG_decrypt(ciphertext, private_key):
    # ElGamal decryption of ciphertext (A,M) with an integer private key
    (A,M) = ciphertext
    b = private_key
...
def EG_sign(string, private_key):
    # Signs the SHA-256 hash of a string with an integer private key
    m = sha_256(string)
    b = private_key
    while True:
        a = randint(1,p-1)
